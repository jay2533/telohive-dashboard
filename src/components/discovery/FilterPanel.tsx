...
interface FilterPanelProps {
  /** Full unfiltered list — used to derive dynamic option sets and ranges. */
  allSpaces: Space[];
  filters: FilterState;
  setFilter: <K extends FilterKey>(key: K, value: ActiveFilters[K]) => void;
  clearAllFilters: () => void;
  /** Whether the panel is open (mobile drawer mode). */
  isOpen: boolean;
  onClose: () => void;
}
  ...
  return (
    <div className="border-b border-gray-100 last:border-0">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        className="flex w-full items-center justify-between py-3.5 text-sm font-semibold text-gray-800 hover:text-gray-900 transition-colors"
      >
        {title}
        <svg
          width="14"
...
function PriceRangeSlider({
  ...
  return (
    <div className="space-y-4">
      {/* Range track */}
      <div className="relative h-1.5 bg-gray-200 rounded-full mx-1 mt-2">
        <div
          className="absolute h-1.5 bg-violet-500 rounded-full"
      ...
      <div>
        <div className="flex justify-between text-xs text-gray-500 mb-1">
          <span>Min</span>
          <span className="font-medium text-gray-700">{fmt(low)}</span>
        </div>
        <input
          type="range"
          min={min}
      ...
      <div>
        <div className="flex justify-between text-xs text-gray-500 mb-1">
          <span>Max</span>
          <span className="font-medium text-gray-700">{fmt(high)}</span>
        </div>
        <input
          type="range"
          min={min}
...
function StarRatingSelector({
  ...
  return (
    <div className="space-y-2">
      <div className="flex gap-1" role="group" aria-label="Minimum star rating">
        {[1, 2, 3, 4, 5].map((star) => (
          <button
            key={star}
            type="button"
...
export function FilterPanel({
  ...
  // ── Panel body (shared between desktop and mobile) ────────────────────────
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
        ...
        <button
          type="button"
          onClick={onClose}
          className="lg:hidden flex h-7 w-7 items-center justify-center rounded-full hover:bg-gray-100 text-gray-500 transition-colors"
        ...
        <CollapsibleSection title="Capacity (people)" defaultOpen={false}>
          <div className="flex items-center gap-2">
            <div className="flex-1">
              <label htmlFor="cap-min" className="text-xs text-gray-500 block mb-1">
                Min
              </label>
              <input
                id="cap-min"
            ...
            <div className="flex-1">
              <label htmlFor="cap-max" className="text-xs text-gray-500 block mb-1">
                Max
              </label>
              <input
                id="cap-max"
  ...
  return (
    <>
      ...
      <aside className="hidden lg:flex lg:flex-col w-64 shrink-0 sticky top-0 h-screen bg-white border-r border-gray-200 overflow-hidden">
        {panelBody}
      </aside>
      ...
      {isOpen && (
        <>
          ...
          />
          {/* Drawer */}
          <aside className="fixed inset-y-0 left-0 z-50 flex flex-col w-72 bg-white shadow-xl lg:hidden">
            {panelBody}
          </aside>
        </>
      )}
